...
resource "azurerm_public_ip" "pip" {
  ...
  name                = lower("pip-vm-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}-0${count.index + 1}")
  location            = data.azurerm_resource_group.existing_rg.location
  resource_group_name = data.azurerm_resource_group.existing_rg.name
  ...
  tags                = merge({ "resourcename" = lower("pip-vm-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}-0${count.index + 1}") }, var.tags, )
...
resource "azurerm_lb" "vmsslb" {
  ...
  name                = var.load_balancer_type == "public" ? lower("lbext-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}") : lower("lbint-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}")
  location            = data.azurerm_resource_group.existing_rg.location
  resource_group_name = data.azurerm_resource_group.existing_rg.name
  sku                 = var.load_balancer_sku
  tags                = merge({ "resourcename" = var.load_balancer_type == "public" ? lower("lbext-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}") : lower("lbint-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}") }, var.tags, )
  ...
  frontend_ip_configuration {
    ...
    subnet_id                     = var.load_balancer_type == "private" ? data.azurerm_subnet.existing_subnet.id : null
  }
...
resource "azurerm_lb_nat_pool" "natpol" {
  ...
  name                           = lower("lbe-nat-pool-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}")
  resource_group_name            = data.azurerm_resource_group.existing_rg.name
...
resource "azurerm_proximity_placement_group" "appgrp" {
  ...
  name                = lower("proxigrp-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}")
  resource_group_name = data.azurerm_resource_group.existing_rg.name
  location            = data.azurerm_resource_group.existing_rg.location
  tags                = merge({ "resourcename" = lower("proxigrp-${var.vmscaleset_name}-${data.azurerm_resource_group.existing_rg.location}") }, var.tags, )
...
resource "azurerm_network_security_group" "nsg" {
  ...
  name                = lower("nsg_${var.vmscaleset_name}_${data.azurerm_resource_group.existing_rg.location}_in")
  resource_group_name = data.azurerm_resource_group.existing_rg.name
  location            = data.azurerm_resource_group.existing_rg.location
  tags                = merge({ "resourcename" = lower("nsg_${var.vmscaleset_name}_${data.azurerm_resource_group.existing_rg.location}_in") }, var.tags, )
...
resource "azurerm_network_security_rule" "nsg_rule" {
  ...
  destination_address_prefix  = element(concat(data.azurerm_subnet.existing_subnet.address_prefixes, [""]), 0)
...
resource "azurerm_virtual_machine_scale_set_extension" "omsagentwin" {
  # count                        = var.deploy_log_analytics_agent && var.log_analytics_workspace_id != null && var.os_flavor == "windows" ? 1 : 0
   count = var.deploy_log_analytics_agent && var.enable_log_analytics_integration && var.os_flavor == "windows" ? 1 : 0
...
resource "azurerm_virtual_machine_scale_set_extension" "omsagentlinux" {
  count = var.deploy_log_analytics_agent && var.enable_log_analytics_integration && var.os_flavor == "linux" ? 1 : 0
...
resource "azurerm_monitor_diagnostic_setting" "nsg" {
 count                      = var.enable_log_analytics_integration ? 1 : 0
  ...
  log_analytics_workspace_id = var.log_analytics_workspace_id  # Ensure this is set
...
resource "azurerm_monitor_diagnostic_setting" "lb-pip" {
  count                      = var.load_balancer_type == "public" && var.enable_load_balancer_diagnostics ? 1 : 0
