...
resource "azurerm_public_ip" "pip" {
  ...
  name                = lower("pip-vm-${var.vmscaleset_name}-${var.region}-0${count.index + 1}")
  location            = var.region
  resource_group_name = var.resource_group_name
  ...
  tags                = merge({ "resourcename" = lower("pip-vm-${var.vmscaleset_name}-${var.region}-0${count.index + 1}") }, var.tags, )
...
resource "azurerm_lb" "vmsslb" {
  ...
  name                = var.load_balancer_type == "public" ? lower("lbext-${var.vmscaleset_name}-${var.region}") : lower("lbint-${var.vmscaleset_name}-${var.region}")
  location            = var.region
  resource_group_name = var.resource_group_name
  sku                 = var.load_balancer_sku
  tags                = merge({ "resourcename" = var.load_balancer_type == "public" ? lower("lbext-${var.vmscaleset_name}-${var.region}") : lower("lbint-${var.vmscaleset_name}-${var.region}") }, var.tags, )
  ...
  frontend_ip_configuration {
    ...
    subnet_id                     = var.load_balancer_type == "private" ? var.public_subnet_id : null
  }
...
resource "azurerm_lb_nat_pool" "natpol" {
  ...
  name                           = lower("lbe-nat-pool-${var.vmscaleset_name}-${var.region}")
  resource_group_name            = var.resource_group_name
...
resource "azurerm_proximity_placement_group" "appgrp" {
  ...
  name                = lower("proxigrp-${var.vmscaleset_name}-${var.region}")
  resource_group_name = var.resource_group_name
  location            = var.region
  tags                = merge({ "resourcename" = lower("proxigrp-${var.vmscaleset_name}-${var.region}") }, var.tags, )
...
resource "azurerm_network_security_group" "nsg" {
  ...
  name                = lower("nsg_${var.vmscaleset_name}_${var.region}_in")
  resource_group_name = var.resource_group_name
  location            = var.region
  tags                = merge({ "resourcename" = lower("nsg_${var.vmscaleset_name}_${var.region}_in") }, var.tags, )
