module "vmss" {
  source = "./modules/vmss"
  public_subnet_address_prefix = module.network.public_subnet_address_prefix[0]
  region = module.network.region_output
  resource_group_name = module.network.resource_group_name_output
  public_subnet_id = module.network.public_subnets[0]
  storage_account_name = "tesing-strogr"
  ...
  log_analytics_workspace_id = module.network.workspace_id

  # Deploy log analytics agents to virtual machine. 
  # Log analytics workspace customer id and primary shared key required.
  deploy_log_analytics_agent                 = true
  log_analytics_customer_id                  = module.network.workspace_customer_id
  log_analytics_workspace_primary_shared_key = module.network.workspace_primary_shared_key
